...
// DATE: 20131212
//
// Calculates 3pt functions and Dq_(x,y,z) continuum derivatives (i.e. "x" in FT) in momentum space using multiple radii for the FT
...
void usage()
{
  cout << endl << "This program calculates 3pt functions and Dq_(x,y,z) continuum derivatives (i.e. factor 'x' in FT) in momentum space using multiple radii for the FT.";
  ...
  cout << endl << "-n (optional) stepsize for radii used for the FT (default = 2) (*)";
  cout << endl << "-c (optional) minimal radius for FT (default = L) (**)";
...
{
  int i = 0;
  while ((i = getopt(argc,argv,"h?c:f:i:m:n:C:")) !=-1)
  {
    switch (i)
    {
      case 'c':
...
int main(int argc, char *argv[])
{
  ...
  {
    cout << endl << "Warning: c_max ('-C') larger than L. Adjusting c_max=L.";
    c_max = corr->get_L();
  }
  if ((c_min==0)&&(c_max==0))
  {
